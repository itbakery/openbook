class Admin::ImagesController < ApplicationController
  ...
  def new
    ...
    @image = @hotel.hotel_images.build
    #@image = Image.new
  ...
  def create
    @hotel = Hotel.find(params[:hotel_id])
    @image = @hotel.hotel_images.build(params[:image])
    #@image = Image.new(params[:image])

    respond_to do |format|
      if @image.save
        #format.html { redirect_to [:admin,@image], notice: 'Image was successfully created.' }
        format.html { redirect_to edit_admin_hotel_path(@hotel), notice: 'Image was successfully created.' }
