class Admin::RoomtypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @roomtype.save
        format.html { redirect_to [:admin,@roomtype], notice: 'Roomtype was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @roomtype.update_attributes(params[:roomtype])
        format.html { redirect_to [:admin,@roomtype] , notice: 'Roomtype was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_roomtypes_url }
      format.json { head :no_content }
    end
  end
end
